    ...
    pub fn descriptor() -> reeve_adapter::Tool {
        reeve_adapter::Tool {
            ...
            description: "Resolve an identity_id to its registered record. \
                ...
                `{identity_id, kind, display_name}` where: \
                \n  - `kind` is one of \"Operator\", \"Agent\", \"External\", \
                or `null` when the identity_id is not registered\n\
                  - `display_name` is the registered name, or `null` when \
                  the identity_id is not registered\n\
                \n\
                When the identity_id is not registered the result is \
                `{identity_id, kind: null, display_name: null}` with \
                is_error=false — this distinguishes \"unknown id\" from a \
                registry fault. \
                \n\nArguments: \
